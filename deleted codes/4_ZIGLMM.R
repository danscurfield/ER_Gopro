...
fit_zipoisson_a = glmmTMB(salmonid_sp ~ tide_height + habitat + (1|site),
  ziformula = ~ 1
  data = salmon,
  family = poisson)
...
fit_zipoisson_b = glmmTMB(salmonid_sp ~ tide_height + (1|site),
                        ziformula = ~ 1, 
                        data = salmon,
                        family = poisson)
...
fit_zipoisson_c = glmmTMB(salmonid_sp ~ tide_height + (1|habitat),
                          ziformula = ~ 1,
                          data = salmon,
                          family = poisson)

#with no random effect or fixed effect for habitat or site
fit_zipoisson_d = glmmTMB(salmonid_sp ~ tide_height, 
                       ziformula = ~1, 
                       data = salmon, 
                       family = poisson)
...
#test *delete below
fit_poisson_b = glmmTMB(salmonid_sp ~ tide_height + (1|site),
                          data = salmon,
                          family = poisson)
